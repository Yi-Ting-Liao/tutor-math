...
\item Now check ordinary convergence using the Alternating Series Test. Let
\[
b_n=\frac1n.
\]
Then \(b_n>0\), and
...
\]
so the positive terms decrease. Also,
\[
\lim_{n\to\infty}b_n=\lim_{n\to\infty}\frac1n=0.
\]
These two facts are exactly the hypotheses of the Alternating Series Test: if \(b_n\) decreases to \(0\), then \(\sum (-1)^n b_n\) converges. Therefore
\[
\sum_{n=1}^{\infty}\frac{(-1)^n}{n}
\]
converges.
...
=\sum_{n=1}^{\infty}\frac{(-1)^n}{n}
\]
Here \(b_n=1/n\) is positive, decreasing, and satisfies \(b_n\to0\). Therefore the Alternating Series Test shows that
\(\sum (-1)^n/n\) converges. However,
\[
\sum_{n=1}^{\infty}\left|\frac{(-1)^n}{n}\right|
=\sum_{n=1}^{\infty}\frac1n
\]
...
\item To evaluate it, first expand the integrand:
\[
(1-2\sin\theta)^2
...
\]
This expansion is useful because each term now has an elementary antiderivative.
...
because
\[
\frac{d}{d\theta}\left(3\theta+4\cos\theta-\sin 2\theta\right)
=3-4\sin\theta-2\cos 2\theta.
\]

\item The outer-loop area is
\[
A_{\text{outer}}
=\frac12\left[F(\theta)\right]_{5\pi/6}^{13\pi/6}.
\]
Now
\[
F\!\left(\frac{13\pi}{6}\right)
=\frac{13\pi}{2}+\frac{3\sqrt3}{2},
\qquad
F\!\left(\frac{5\pi}{6}\right)
=\frac{5\pi}{2}-\frac{3\sqrt3}{2}.
\]
Therefore
\[
A_{\text{outer}}
...
\item The inner-loop area is
\[
A_{\text{inner}}
=\frac12\left[F(\theta)\right]_{\pi/6}^{5\pi/6}.
\]
Since
\[
F\!\left(\frac{5\pi}{6}\right)
...
\[
A_{\text{inner}}
=\frac12\left(2\pi-3\sqrt3\right)
=\pi-\frac{3\sqrt3}{2}.
\]

\item Finally subtract the inner loop from the outer loop:
\[
...
So use
\[
F(\theta)=\int(1-2\sin\theta)^2\,d\theta
...
&=\frac12\left[F(\theta)\right]_{5\pi/6}^{13\pi/6}  \\
&=\frac12\left[
\left(\frac{13\pi}{2}+\frac{3\sqrt3}{2}\right)
-\left(\frac{5\pi}{2}-\frac{3\sqrt3}{2}\right)
\right] \\
&=2\pi+\frac{3\sqrt3}{2},
\end{aligned}
\]
...
&=\frac12\left[
\left(\frac{5\pi}{2}-\frac{3\sqrt3}{2}\right)
-\left(\frac{\pi}{2}+\frac{3\sqrt3}{2}\right)
\right] \\
&=\pi-\frac{3\sqrt3}{2}.
\end{aligned}
\]
...
with
\[
(1-2\sin\theta)^2=3-4\sin\theta-2\cos2\theta,
\qquad
F(\theta)=3\theta+4\cos\theta-\sin2\theta.
\]
Thus
\[
\begin{aligned}
